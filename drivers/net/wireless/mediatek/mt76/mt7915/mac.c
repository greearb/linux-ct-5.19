...
void mt7915_coredump_work(struct work_struct *work)
{
	...
	if (time_is_after_jiffies(dev->coredump.last_activity +
		...
		dev_err(dev->mt76.dev, "mtk7915: coredump detected, queue for later.");
	...
	dev_err(dev->mt76.dev, "mtk7915: coredump detected, processing, dump: %p", dump);
...
static int mt7915_dfs_start_rdd(struct mt7915_dev *dev, int chain)
{
	...
	err = mt76_connac_mcu_rdd_cmd(&dev->mt76, RDD_DET_MODE, chain,
				      MT_RX_SEL0, 1);
	dev_info(dev->mt76.dev, "dfs-start-rdd, RDD_DET_MODE rv: %d", err);
	return err;
}
